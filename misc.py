...
def gray_to_display_surface(gray, display_size):
    """
    Generate pygame surface given an gray image float32 matrix
    :param gray: gray image float32 matrix
    :param display_size: display size
    :return: pygame surface
    """
    surface = pygame.Surface((display_size, display_size)).convert()
    display = skimage.transform.resize(gray, (display_size, display_size))
    display = np.flip(display, axis=1)
    display = np.rot90(display, 1)
    pygame.surfarray.blit_array(surface, display)
    return surface
